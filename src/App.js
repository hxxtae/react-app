...
import { Subject, TOC, Control, ReadContent, CreateContent } from './components';
import './App.css';
...
class App extends Component {
  ...
  getContent() {
    ...
    if (this.state.mode === 'welcome') {
      ...
      _article = <CreateContent title={_title} desc={_desc} onSubmit={this.onSubmit}></CreateContent>;
    }
    return _article;
  }
  ...
  onSubmit = (title, desc) => {
    ...
    const listUpdate = this.state.toc.concat(
      {
        id: tocLen + 1,
        title,
        desc,
...
/*
  ...
  ※ 요약하자면
    ...
    state는 하위 컴포넌트에서 상위 컴포넌트로 데이터 전달 가능 (상위 state 나 함수선언 에 이벤트를 심어서 가능 - setState())
                                                            (setState 는 여기(상위 컴포넌트)에서 선언해야 동작한다.)
